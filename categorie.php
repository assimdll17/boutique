...
$statement = $pdo->query($sql_get_categ);
...
if ($categs_array) {
    foreach ($categs_array as $categ) {
        //var_dump($product);
        $sql_get_count = "SELECT Count(SKU) AS `count` FROM `prod_cat` WHERE id='".$categ['id']."'";
        $statement2 = $pdo->query($sql_get_count);
...
function renderCategoryHTML($categ, $categ_count){
    ...
    <li class='list-group-item d-flex justify-content-between align-items-start'>
        <div class='ms-2 me-auto'>
          <div class='fw-bold'><a href='index.php?categor=".$categ['id']."'>".$categ['name']."</a></div>
        </div>
